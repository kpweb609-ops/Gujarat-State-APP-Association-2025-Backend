...
router.post('/', upload.single('Release_photo'), async (req, res) => {
  try {
    ...
    if (req.file) {
      Release_photo = await uploadToCloudinary(req.file.buffer, req.file.mimetype);
    }
...
router.put('/:id', upload.single('Release_photo'), async (req, res) => {
  try {
    ...
    if (req.file) {
      release.Release_photo = await uploadToCloudinary(req.file.buffer, req.file.mimetype);
